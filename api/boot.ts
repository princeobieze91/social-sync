...
app.get("/api/test-fb", async (c) => {
  ...
  try {
    ...
    const meRes = await fetch(`https://graph.facebook.com/v25.0/me?fields=name,id&access_token=${token}`);
    results.user = await meRes.json();

    const pagesRes = await fetch(`https://graph.facebook.com/v25.0/me/accounts?fields=name,id,access_token&access_token=${token}`);
    ...
    if (results.pages?.data?.[0]) {
      ...
      const igRes = await fetch(`https://graph.facebook.com/v25.0/${pageId}?fields=instagram_business_account&access_token=${pageToken}`);
